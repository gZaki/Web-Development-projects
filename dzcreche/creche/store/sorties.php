...
<![endif]-->        <meta http-equiv="Content-Type" content="text/html; charset=ISO-8859-1" />
    ...
    <body>
        <div id="toutcontenu">
            ...
            <!--Content -->
            <div id="contenu">
                <img id="sorties" src="img/nossorties.png" alt="Nos sorties" title="Nos sorties"/>
                <div id="texteenseigner"> <!--backgroundrectange blanc-->
                    <div class="contenuenseigner"> <!-- contenu texte du rectangle-->
                        <?php echo htmlspecialchars(stripslashes($row['sortie'])); ?>

                    </div> <!--fin contenu texte du rectangle-->
                </div> <!--fin backgroundrectangleblanc-->
                <div class="clear"></div>
                <div id="textesortiesdroite"> <!--backgroundrectange blancdroite-->
                    <div class="contenusorties"> <!-- contenu texte du rectangle droite-->
                        <?php echo htmlspecialchars(stripslashes($row['sortie2'])); ?>
                    </div> <!--fin contenu texte du rectangle de droite-->
                </div> <!--fin backgroundrectangleblanc-->
                <!--slide photo-->
                <!--<div id="slidephoto">
                        <img src="img/chasseautresor019.jpg"/>
                </div>-->
                <!-- Galerie photo -->
                <div class="gallery" id="gal_sorties">
                                      <?php 
                $query="SELECT * FROM store_sorties WHERE id_store='$id'";
                $result=mysqli_query($con,$query);
                if (mysqli_num_rows($result) > 0) {
                    while($row=mysqli_fetch_assoc($result)){?>
                                <a href="#"><img src="img/sorties/<?php echo $row['nom']; ?>" class="imgs_anim start" style="width:100%" alt="" /></a>
                                                <?php
                    }
                }
                ?>
                ...
                if (mysqli_num_rows($result) > 0) {
                    while($row=mysqli_fetch_assoc($result)){?>
                                <li><img src="img/sorties/<?php echo $row['nom']; ?>" style="width:100%" alt="" /></li>
                                                <?php
                    }
                }
                ?>
                        <!--<li>
                            <img  src="img/sorties/<?php ?>" alt="" />
                        </li>-->
                    </ul>
                </div>
                <!--footer avec poussins-->
                <img id="poussinballonbl" style="top:620px;" src="img/poussinballonbl.png" alt="" title=""/>
                <img id="petiteherbesorties" style="top:780px;" src="img/petiteherbe.png" alt="" title=""/>
            </div><!--fin contenu -->
        </div> <!--balisefinale-->        <script language="javascript" type="text/javascript" src="js/jquery.flexslider-min.js"></script>
